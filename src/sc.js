...
function runControl(control, status) {
  return runPoll(
    control,
    function (command) {
      return query(control.server, { name: command.service });
    },
    function (services) {
      return services[0].state.code === status;
    },
    function (command) {
      return "Timed out attempting to " + control.command + " " + command.service + ".";
    }
  );
}
...
module.exports.stop = function () {
  ...
  return runPoll(
    command,
    function (command) {
      return runTasklist(tasklist(command.service, command.server));
    },
    function (processes) {
      return processes.length == 0;
    },
    function (command) {
      return "Timed out waiting for the " + command.service + " service process to terminate.";
    }
  );
};
...
module.exports.setSidType = function () {
  return runSc(sc.setSidType(arguments));
};
