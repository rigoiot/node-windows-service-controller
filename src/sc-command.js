...
function getArgs(args, fixed) {
  ...
  var server, options;
  if (_.isObject(_.last(argsIn)) && !_.isArray(_.last(argsIn))) options = argsIn.pop();
...
function buildCommand(commandName, server, argsIn, successCodes) {
  ...
  if (argsIn) {
    if (_.isArray(argsIn)) argsOut.push.apply(argsOut, argsIn);
...
exports.setFailureConfig = function (args) {
  ...
  if (options.actions) {
    var actions = [];
    if (options.actions.restart) actions.push("restart", options.actions.restart);
    if (options.actions.run) actions.push("run", options.actions.run);
    if (options.actions.reboot) actions.push("reboot", options.actions.reboot);
    if (actions.length > 0) addArg(argsOut, "actions", toSlashSeperated(actions));
  }
  return buildCommand("failure", argsIn.server, argsOut);
};

exports.setSidType = function (args) {
  return buildSimpleCommand("sidtype", args, 3);
};
